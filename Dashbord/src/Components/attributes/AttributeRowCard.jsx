...
import { useDeleteAttributeByIdMutation } from "../../api/attribute.api";
import { toast } from "react-toastify";
import { allowed } from "../../helper/roles";
import { useSelector } from "react-redux";
const AttributeRowCard = ({ item, index, handleOpenEditModal }) => {
  const { colors, customColors, palette } = useTheme();
  const [_, { language: lang }] = useTranslation();
  const [deleteAttributeById] = useDeleteAttributeByIdMutation();
  const { role } = useSelector((state) => state.user);

  const handleRemoveAttibute = () => {
    deleteAttributeById(item._id)
      .unwrap()
      .then((res) => {
...
export default AttributeRowCard;